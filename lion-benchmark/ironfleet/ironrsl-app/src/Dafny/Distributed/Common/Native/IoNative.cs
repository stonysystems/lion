...
namespace Native____Io__s_Compile {
  ...
  public partial class NetClient
  {
    ...
    internal LionIoScheduler lionScheduler;
    internal bool useLion;
    ...
    internal NetClient(LionIoScheduler i_lionScheduler, byte[] publicKeyHash)
    {
      scheduler = null;
      lionScheduler = i_lionScheduler;
      useLion = true;
      myPublicKeyHash = Dafny.Sequence<byte>.FromArray(publicKeyHash);
    }
    ...
    public static NetClient CreateWithLion(PrivateIdentity myIdentity, string localHostNameOrAddress, int localPort,
                                            List<PublicIdentity> knownIdentities, bool verbose, bool useSsl)
    {
      try
      {
        var lionScheduler = LionIoScheduler.CreateServer(myIdentity, localHostNameOrAddress, localPort,
                                                          knownIdentities, verbose, useSsl);
        var keyHash = lionScheduler.HashPublicKey(
          IoScheduler.GetCertificatePublicKey(
            new System.Security.Cryptography.X509Certificates.X509Certificate2(
              myIdentity.Pkcs12, "", System.Security.Cryptography.X509Certificates.X509KeyStorageFlags.Exportable)));
        return new NetClient(lionScheduler, keyHash);
      }
      catch (Exception e)
      {
        System.Console.Error.WriteLine(e);
    ...
    public void Receive(int timeLimit, out bool ok, out bool timedOut, out Dafny.ISequence<byte> remote, out byte[] buffer)
    {
      byte[] remoteBytes;
      if (useLion)
        lionScheduler.ReceivePacket(timeLimit, out ok, out timedOut, out remoteBytes, out buffer);
      else
    ...
    {
      if (useLion)
        return lionScheduler.SendPacket(remote.Elements, buffer);
      else
    ...
    {
      if (useLion)
        return lionScheduler.HashPublicKey(key);
      else
