...
namespace IronRSLServer
{
  public class Params
  {
    ...
    public bool UseLion { get { return useLion; } }
    ...
    private bool SetValue(string key, string value)
    {
      ...
      if (key == "lion") {
        return SetBoolValue(key, value, ref useLion);
      }
...
");
      ...
      Native____Io__s_Compile.NetClient nc;
      if (ps.UseLion)
      {
        Console.WriteLine("Using Lion async IO scheduler");
        nc = Native____Io__s_Compile.NetClient.CreateWithLion(privateIdentity, ps.LocalHostNameOrAddress, ps.LocalPort,
                                                               serviceIdentity.Servers, ps.Verbose, serviceIdentity.UseSsl);
      }
      else
      {
        nc = Native____Io__s_Compile.NetClient.Create(privateIdentity, ps.LocalHostNameOrAddress, ps.LocalPort,
                                                       serviceIdentity.Servers, ps.Verbose, serviceIdentity.UseSsl);
      }
