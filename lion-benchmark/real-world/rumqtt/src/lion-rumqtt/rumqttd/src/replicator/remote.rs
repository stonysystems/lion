...
use std::{io, thread};
use lion::net::TcpListener;
use lion::JoinHandle;
use lion::time::Elapsed;
...
impl Cluster {
            ...
            let network =
                match Cluster::network_connect(remote_addr, &connector_id.to_string()).await {
                    ...
                    Err(e) => {
                        ...
                        lion::time::sleep(Duration::from_secs(1)).await;
                        continue;
                    }
                };
    ...
    /// Start the cluster
    #[lion::main(flavor = "current_thread")]
