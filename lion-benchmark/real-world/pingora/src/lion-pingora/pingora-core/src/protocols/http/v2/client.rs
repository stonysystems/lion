...
async fn do_ping_pong(
    ...
    lion::time::sleep(interval).await;
    loop {
        ...
        match lion::time::timeout(PING_TIMEOUT, ping_fut).await {
            Err(_) => {
...
mod tests_h2 {
    ...
    async fn h2_body_bytes_received_multi_frames() {
        ...
        let (closed_tx, closed_rx) = lion::sync::watch::channel(false);
