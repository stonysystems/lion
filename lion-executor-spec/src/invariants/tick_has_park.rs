...
// TICK_HAS_PARK: Every Tick::End has a Park before it (in same tick)
...
pub open spec fn validity_fn(l: Log, i: int) -> bool {
  exists |p: int| #![auto]
    0 <= p < i && is_park_at(l, p) && no_tick_begin_between(l, p, i)
}

pub open spec fn tick_has_park() -> ActionSafety<Log> {
  ActionSafety {
