...
// TICK_HAS_DRAIN_DEFERRED: Every Tick::End has a Drain(Deferred) before it (in same tick)
...
pub open spec fn validity_fn(l: Log, i: int) -> bool {
  exists |d: int| #![auto]
    0 <= d < i && is_drain_deferred_at(l, d) && no_tick_begin_between(l, d, i)
}

pub open spec fn tick_has_drain_deferred() -> ActionSafety<Log> {
  ActionSafety {
