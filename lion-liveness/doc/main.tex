...
\midrule
End-to-end theorem (\S\ref{sec:theorem}) & \lstinline|end_to_end_liveness_env_N_trace| (proven by \lstinline|..._holds|) & \texttt{composed/proof/assumption\_satisfiable.rs} \\
Arbitrary-depth variant & \lstinline|end_to_end_liveness_from_depth_trace| & \texttt{composed/proof/depth\_generalization.rs} \\
Explicit step bound ($\mathit{chunk}+\mathit{cap}\cdot\mathit{chunk}$) & \lstinline|ete_reaches_goal_explicit_bound| & \texttt{composed/proof/assumption\_satisfiable.rs} \\
Well-formedness preservation & \lstinline|composed_progress_preserves_wf_lemma| & \texttt{composed/proof/end\_to\_end.rs} \\
Assumptions jointly satisfiable & \lstinline|top_theorem_precondition_satisfiable|, \lstinline|env_N_satisfiable| & \texttt{composed/proof/assumption\_satisfiable.rs} \\
Non-vacuity: timer path & \lstinline|b_domain_inhabited| (all budgets: \lstinline|..._at_any_n|) & \texttt{composed/proof/inhabitation\_goal\_wake.rs}, \texttt{inhabitation\_budget.rs} \\
Non-vacuity: io path & \lstinline|bio_domain_inhabited| & \texttt{composed/proof/inhabitation\_goal\_io.rs} \\
Non-vacuity: deferred path & \lstinline|d_domain_inhabited| & \texttt{composed/proof/inhabitation\_goal\_defer.rs} \\
Non-vacuity: cross-task path & \lstinline|t_domain_inhabited| & \texttt{composed/proof/inhabitation\_goal\_taskwake.rs} \\
Non-vacuity: depth variant & \lstinline|depth_domain_inhabited| & \texttt{composed/proof/inhabitation\_depth.rs} \\
Cross-task arrival clause non-vacuous & \lstinline|taskwake_arrival_within_nonvacuous_witness| & \texttt{composed/proof/inhabitation\_goal\_taskwake.rs} \\
Drain occurrence derived & \lstinline|single_progress_has_drain_*| & \texttt{composed/proof/assumption\_satisfiable.rs} \\
\bottomrule
\end{tabular}
