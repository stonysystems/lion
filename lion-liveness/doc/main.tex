...
\begin{itemize}
  ...
  \item \lstinline|task_logs| maps task ids to each task's own log of
    interactions with the synchronization primitives---the \emph{Tasks} managed by
    the Core. The map is append-only: every task the executor has polled has an
    entry, and entries are never removed---a completed task's log is simply
    retained.
...
observes. So a step must carry every fired-but-unconsumed wake through to a poll:
  ...
  \end{itemize}
We model each pipeline as a trusted boundary rather than a state object shared by two
modules: a queue enters the model only as its log-derived \emph{pending set}---the wakes
fired but not yet taken by a drain---and a step requires every pending wake to be included
in this step's drain of that queue, which takes the set wholesale. No intra-queue order is
modeled (none is needed: a drain consumes everything at once); the FIFO order that
scheduling fairness relies on lives in the executor's runnable queue, not in these
pipelines. The queues are internal to the runtime and never disturbed from outside, so
this set-level capture is faithful while sidestepping the semantics of cross-module shared
state.
\end{enumerate}

\noindent
This step relation is the \lstinline|progress| field of the composed module spec, so the
schema's $\mathit{progress}^{\,n}_{\mathit{asm}}$ is its $n$-fold composition. Note that
conditions 2--3 constrain the \emph{successor} state with the module invariants and the
alignment; Section~\ref{sec:theorem} returns to what this means for well-formedness
preservation.
...
exactly why they must be assumed rather than proved. (Alongside these, the mechanized
environment filter records two finiteness facts of any physical execution rather than
genuine assumptions about the outside world: timestamps are non-negative, and the runnable
queue's length never exceeds a symbolic constant---the $C$ that reappears in the explicit
bound of Section~\ref{sec:theorem}.)
...
\begin{itemize}
  ...
  \item \emph{Well-formedness is inductive---by construction.} The composed step relation
    itself carries the post-state invariants as conjuncts (each module's step requires that
    module's invariant at the successor, and the alignment clauses supply the cross-module
    part), so \texttt{progress\_preserves\_wf} holds definitionally rather than by a
    separate inductive argument, and the precondition on $s$ propagates along the whole run
    for free. The corresponding obligation is discharged where it belongs: the
    implementation crates prove that the executable executor and reactor actually take such
    invariant-carrying steps.
...
\addlinespace
Utility  & \vcode{passwaker_to_wakewaker_contract} & Once a task hands its waker to a synchronization primitive (a channel, a lock, \dots), that primitive invokes the waker. \\
\bottomrule
\end{tabular}
...
(\lstinline|valid_task_polling|) and only from inside a tick
(\lstinline|poll_within_tick|), and any task that is runnable is actually polled within the
...
\lstinline|wake_on_io_ready_writable|), produces a wake. A further group keeps the IO
registration cycle honest---an IO register or deregister syscall is issued only inside the
corresponding inbound API cycle (\lstinline|register_io_in_cycle|,
\lstinline|deregister_io_in_cycle|), the result that cycle returns matches the syscall it
actually performed (\lstinline|inbound_register_io_result|,
\lstinline|inbound_deregister_io_result|), and a readiness report only arises inside a park
cycle (\lstinline|io_ready_in_park|). In all, the reactor carries 19 such properties, split
between \texttt{reactor\_inv} and the extended \texttt{reactor\_ext\_inv}.
...
(paths relative to \vcode{lion-liveness/src/}; the module contracts of
Table~\ref{tab:contracts} live in \vcode{lion-reactor-spec/src/contracts/},
\vcode{executor/contracts/}, and---for the utility contract---%
\vcode{lion-utility/lion-utility-spec/src/generic/contract.rs}).
...
Drain occurrence derived & \vcode{single_progress_has_drain_*} & \vcode{executor/proof/bounded_drain_poll.rs} \\
\bottomrule
\end{tabular}
